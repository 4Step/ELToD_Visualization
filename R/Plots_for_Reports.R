...
plot_diverted_shares <- function(data, plot_title) {
  # Axes settings
  ...
  data %>%
     ...
     layout(title = plot_title, # "Diverted Percent (EL Share)",
...
# Using plotly 
diverted_el_shares <- df %>% filter(Policy == "newPolicy", Year == 2040, Seg == 1) %>%
       plot_diverted_shares("Diverted Percent (EL Share)")

# save it as pdf 
# export(diverted_el_shares, paste0(dir_path,"/plot.png"))
